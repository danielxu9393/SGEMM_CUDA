// Start from 1017, try vectorizing loading shared -> registers
// Is the exact same as 1017, I assume it is because the loop is so simple it is automatically vectorized!!
// Makes sense!! But good to just sanity check lmfao
#pragma once
...
void sgemmLab4RegVectorizeSharedToReg(int M, int N, int K, float alpha,
    ...
    for (int bk = 0; bk < K; bk += BK) {
        ...
        for (int r_t = 0; r_t < BK; ++r_t) {
            ...
            // Vectorized load of a row from As into regA
            for (int i = 0; i < TM; i += 4) {
                int col = threadRow * TM + i; // starting column in the shared tile
            ...
            for (int j = 0; j < TN; j += 4) {
                int row = r_t;                // current row in the shared tile
                int col = threadCol * TN + j;   // starting column in the shared tile
...
void runSgemmLab4RegVectorizeSharedToReg(int M, int N, int K, float alpha, float *A, float *B,
    float beta, float *C) {
    const uint BK = 8; // 16
    const uint TM = 8; // 10
    const uint TN = 8;
    if (M >= 128 and N >= 128) {
        ...
        sgemmLab4RegVectorizeSharedToReg<BM, BN, BK, TM, TN>
