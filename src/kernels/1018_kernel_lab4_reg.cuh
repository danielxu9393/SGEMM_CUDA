// Make it 2D TID again (Slows 4096 from 18.6TFlops -> 18.3TFlops)
#pragma once
...
void sgemmLab4RegVectorize2DTid(int M, int N, int K, float alpha,
    ...
    // blockDim.x == BN / TN and blockDim.y == BM / TM
    // BM = BN, TM = TN

    const uint tid = threadIdx.x + threadIdx.y * blockDim.x;
    const uint innerRowA = primitiveWidth * tid / BK;
    const uint innerColA = (primitiveWidth * tid) % BK;
    const uint strideA = primitiveWidth * numThreadsBlocktile / BK;

    // Nvm, this does nothing lmfao...
    // const uint innerRowB = primitiveWidth * (threadIdx.x + threadIdx.y * blockDim.x) / BN;
    // const uint innerRowB = primitiveWidth * (threadIdx.x / BN + threadIdx.y / TN);
    // primitiveWidth*blockDim.x <= BN
    // const uint innerRowB = primitiveWidth * threadIdx.y / TN;

    const uint innerRowB = primitiveWidth * tid / BN;
    const uint innerColB = (primitiveWidth * tid) % BN;
...
void runSgemmLab4RegVectorize2DTid(int M, int N, int K, float alpha, float *A, float *B,
    float beta, float *C) {
    const uint BK = 8; // 16
    const uint TM = 8; // 10
    const uint TN = 8;
    if (M >= 128 and N >= 128) {
        ...
        sgemmLab4RegVectorize2DTid<BM, BN, BK, TM, TN>
