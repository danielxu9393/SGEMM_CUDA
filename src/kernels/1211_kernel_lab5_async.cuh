// From kernel 1113, just initial experiment to add async global loading makes it faster than kernel 10!!!
// 22.2TFlops while kernel 10 is 21.7Tflops!!!
// I need 3 blocks per SM for max performance on A6000
#pragma once
...
namespace async_common {
    ...
    __device__ void loadFromGmem(int N, int K, const float *A, const float *B, float *As,
        ...
        for (uint offset = 0; offset < BM; offset += strideA) {
            // float4 tmp = *reinterpret_cast<const float4*>(&A[(innerRowA + offset) * K + innerColA]);
            // As[(innerColA + 0) * BM + (innerRowA + offset)] = tmp.x;
            // As[(innerColA + 1) * BM + (innerRowA + offset)] = tmp.y;
            // As[(innerColA + 2) * BM + (innerRowA + offset)] = tmp.z;
            // As[(innerColA + 3) * BM + (innerRowA + offset)] = tmp.w;
...
void sgemmLab5AsyncLoad(int M, int N, int K, float alpha,
    ...
    __shared__ float A_buffer[BM * BK];
    ...
    for (int bk = 0; bk < K; bk += BK) {
        async_common::loadFromGmem<BM, BN, BK, strideA, strideB>(
            N, K, A, B, A_buffer, Bs, innerRowA, innerColA, innerRowB, innerColB);

        async_common::async_memcpy_waitall();

        async_common::transposeA<BM, BK, BK, strideA>(N, K, A_buffer, As, innerRowA, innerColA);
...
void runSgemmLab5AsyncLoad(int M, int N, int K, float alpha, float *A, float *B,
    float beta, float *C) {
    const uint BK = 16; // 16
    const uint WMITER = 1;
    const uint WNITER = 4;
    ...
    sgemmLab5AsyncLoad<BM, BN, BK, TM, TN, NWM, NWN, WMITER, WNITER>
