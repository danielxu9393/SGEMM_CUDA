...
void sgemmLab4RegWarpTiling(int M, int N, int K, float alpha,
...
void runSgemmLab4RegWarpTiling(int M, int N, int K, float alpha, float *A, float *B,
    float beta, float *C) {
    const uint BK = 16; // 16
    const uint WMITER = 1;
    const uint WNITER = 4;
    ...
    if (M >= 128 and N >= 128) {
        ...
        sgemmLab4RegWarpTiling<BM, BN, BK, TM, TN, NWM, NWN, WMITER, WNITER>
